...
Feng's Adaptive RED
===================
Feng's Adaptive RED is variant of RED with a feature that it adapts maximum drop
probability. The model in ns-3 contains implementation of this feature, and is a
port of ns-2 Feng's Adaptive RED model. Note that the user is allowed to choose
and explicitly configure the simulation by selecting feature.
...
set to true, as done in ``src/traffic-control/examples/red-vs-fengadaptive.cc``:
